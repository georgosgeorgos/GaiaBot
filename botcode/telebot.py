...
class Secretary(object):
    ...
    def handle_message(self, msg):
        user_id = msg['from']['id']
        user = self.db.find_by_tid(guy_id)

        try:
            if user['id'] in self.expected_answers:
                return self.expected_answers[user_id](msg)
        except KeyError:
            self.log_message("if user['id'] in self.expected_answers:, failed" + repr(msg))
...
def look_for_specialist(self, params, querier=None):
    found = False
    for employee in self.db.find_by_job(params['job']):
        found = self.query_employee(employee)
        if found:
            break
...
def query_employee(self, message_offset_id, person, bot_obj, query_msg=None):
    his_answer = False
